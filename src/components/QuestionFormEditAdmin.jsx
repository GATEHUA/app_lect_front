...
import { updateQuestionxDocenteEvalSchema } from "../schemas/question";
import { createPreguntaRequest, updatePregunaRequest } from "../api/preguntas";
import { toast } from "sonner";
// import { useParams } from "react-router-dom";
import { useUserStore } from "../store/UserStore";

export const QuestionFormEditAdmin = ({
  getMyQuestions,
  dataQuestion,
  ...
  const defSchema = useRef({
    resolver: zodResolver(updateQuestionxDocenteEvalSchema),
  });
  useEffect(() => {
    if (dataQuestion) {
      ...
      setValue("puntajepregunta", dataQuestion.puntajepregunta);
      setValue("estadoAceptacion", dataQuestion.estadoAceptacion);
    }
    // else {
    //   setValue("contenido", "");
    //   setValue("esquema", "");
    ...
    <div>
      <form onSubmit={onSubmit}>
        <div className="flex flex-col space-y-1.5">
          ...
          <div>
            <Label htmlFor="puntajepregunta" className="dark:text-white mb-2">
              Puntos por la creación de la pregunta
            </Label>
            <InputText
              {...register("puntajepregunta", { valueAsNumber: true })}
              disabled={user.rol == "Usuario"}
              type="number"
              message={errors.puntajepregunta?.message}
              id="puntajepregunta"
              placeholder="Pnts."
              className="p-1"
            />
        ...
        <div>
          <Label htmlFor="estadoAceptacion" className="dark:text-white mb-2">
            Estado de Aceptación
          </Label>
          <select
            id="estadoAceptacion"
            className="bg-gray-50 border-[1.7px]  text-gray-900 rounded-lg block w-full dark:bg-gray-600 dark:placeholder-gray-400 dark:text-white border-gray-300 focus:ring-blue-500 focus:border-blue-500  dark:border-gray-500 p-1.5"
            {...register("estadoAceptacion")}
          >
            <option value="Si">Si</option>
            <option value="No">No</option>
          </select>
          {errors.estadoAceptacion && (
            <span className="text-red-500">
              {errors.estadoAceptacion.message}
            </span>
          )}
        </div>
